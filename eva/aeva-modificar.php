...
function borraAutoEvaluacion()
{
  ...
  $qry1 = "DELETE FROM respuestas WHERE func_rut = '$full_rut' AND res_evaluador='$full_rut'";
  $qry3 = "DELETE FROM metas WHERE 'func_rut' = '$full_rut' AND meta_evaluador = '$full_rut'";
  $qry2 = "
  DELETE
  FROM planificacion
  ...
  SELECT plan_id
  FROM metas
  WHERE func_rut = '$full_rut' AND meta_evaluador = '$full_rut')";
  $qry4 = "UPDATE funcionario SET func_aevaluado=0, func_fecha_ae=NULL WHERE func_rut='$full_rut' LIMIT 1";
...
foreach ($_POST as $clave => $valor)
{
    if (startsWith($clave, "frm") != TRUE AND startsWith($clave, "plan") != TRUE) //Solo Respuestas!
    {
        if (substr($clave,0,2) == "c_")
        {
            ...
            $qry2 = "UPDATE respuestas SET res_comentario = '$valor' WHERE preg_id = '$id2' AND func_rut ='$rut'";
            InsertaDB($qry2);
            continue;
        }
        $qry1 = "INSERT INTO respuestas VALUES (NULL, '$rut', '$clave', '$valor', NULL, '$rut')";
    ...
    if (startsWith($clave, "plan_text_meta") == TRUE) //Solo Planificacion
    {
        ...
        if ($contador == 1)
        {
            if ($_POST['plan_text_meta1'] != '')
            {
                ...
                $qry2 = "INSERT INTO metas VALUES (NULL, '$last_id', '$rut', '$rut')";
...
$qry = "UPDATE funcionario SET func_aevaluado = '1', func_fecha_ae = '$date' WHERE func_rut = '$rut'";
console_log("QRY: ".$qry);
$mysqli = new mysqli(DB_SERVER2, DB_USERNAME2, DB_PASSWORD2, DB_NAME2); 
$mysqli->set_charset("utf8mb4");
